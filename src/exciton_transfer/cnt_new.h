...
private:
  ...
  float chirality_check; //random number to determine chirality
  int _n1, _n2;
  int _m1,_m2;
...
public:
  ...
  cnt(const nlohmann::json j, const std::string parent_directory)
  {
    ...
    // set chirality
    std::array<int,4> chirality = j["chirality"];
    chirality_check = rand() % 10;
    ...
    _n = _n2;
    _m = _m2;
    if (chirality_check < 5)
    {
        _n = _n1;
        _m = _m1;
    }
    std::cout << "chirality is: " << "[" << _n <<"," << _m <<"]" << "\n";
